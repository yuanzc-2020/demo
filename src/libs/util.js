...
import { getMenuList, getPermissionList } from '@/api/card-system-api';
import lazyLoading from './lazyLoading.js';
import router from '@/router/index';
import Cookies from "js-cookie";
import globalUtil from './globalUtil.js'
import { getCookieStore } from './storage'
...
util.initRouter = function (vm) {
    ...
    const otherRouter = [{
        ...
        component: 'card-common/common-error-page/404'
    }];
    // 判断用户是否登录
    let loginSessionId = getCookieStore('loginSessionId')
    if (!loginSessionId) {
        return;
    }
    if (!vm.$store.state.app.added) {
        ...
        //                 level: e.level == 1 ? "0" : e.level == 2 ? "1" : "2",
        //                 type: e.vueType == 0 ? "-1" : e.vueType == 1 ? "0" : "1",
        ...
        //         })
        //         //生成树形数据
        //         let menuData = globalUtil.listConvertTreeData(
        //             menus,
        //             'id',
        //             'parentId',
        //             'children'
...
util.initRouterNode = function (routers, data) {
    ...
    for (var item of data) {
        ...
        meta.title = menu.title ? menu.title + " - 发票管理系统" : null;
