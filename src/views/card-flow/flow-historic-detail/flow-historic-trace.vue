...
<script>
import { getBusHighlightImg, getBusTaskHistory } from '@/api/card-flow-api';
...
export default {
	...
	methods: {
		init() {
			this.businessId = this.traceId;
			this.imgUrl =
				getBusHighlightImg + this.businessId + '?time=' + new Date();
			this.getDataList();
		},
		getDataList() {
			this.loading = true;
			getBusTaskHistory(this.businessId).then(res => {
				this.loading = false;
				if (res.success) {
