				...
				<div class="header-middle-con">
					...
					>
						...
						<!-- 遍历navList.slice中的数据 -->
							...
							<Submenu
								name="sub"
								v-if="navList.length > sliceNum"
							>
								<template slot="title">更多</template>
								<MenuItem
									class="main-nav-menu-item"
									v-for="(item, i) in navList.slice(
										sliceNum,
										navList.length
									)"
									:key="i"
...
import { ws, logout } from '@/api/card-system-api';
...
export default {
	...
	methods: {
		...
		handleClickUserDropdown(name) {
			if (name == 'ownSpace') {
			...
			} else if (name == 'changeUnit') {
				this.setStore('loginUserInfo', '');
				...
				logout().then((res) => {
					...
					this.removeStore('loginUserInfo');
					// 强制刷新页面 重新加载router
					location.reload();
				});
			}
	...
	created() {
		...
		// 初始化全局数据字典
		if (getStore('loginUserInfo')) {
			let userInfo = JSON.parse(getStore('loginUserInfo'));
			if (userInfo) {
				this.$store.commit('setCurrentUserInfo', userInfo);
				console.log('=== Main Initialized Global Data ===');
